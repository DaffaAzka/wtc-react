  ...
  return (
    <Link to={`/student/tracks/${track.slug}`} className="group block">
      <div className="overflow-hidden rounded-2xl bg-white border border-gray-200 dark:bg-[#0b1215] dark:border-white/10 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300 h-full flex flex-col">
        {/* Image */}
        <div className="relative h-44 w-full overflow-hidden" style={{ background: getPatternBackground(track.title) }}>
          {track.image_url && (
            <img
              src={track.image_url}
              alt={track.title}
        ...
        <div className="flex flex-col flex-1 p-5 space-y-3">
          <h3 className="font-extrabold text-[15px] text-gray-900 dark:text-white group-hover:text-[#1c81ff] transition-colors line-clamp-2 leading-snug" style={{ letterSpacing: "-0.01em" }}>
            {track.title}
          </h3>

          {track.description && <p className="text-[13px] leading-relaxed text-gray-500 dark:text-gray-400 line-clamp-2 flex-1">{track.description}</p>}

          <div className="flex items-center justify-between pt-2 border-t border-gray-100 dark:border-white/5">
            <span className="text-[12px] font-bold text-gray-400 dark:text-gray-600">{isEnrolled ? "Lanjutkan belajar" : "Mulai sekarang"}</span>
            <div className={`flex items-center gap-1 text-[13px] font-bold transition-colors ${isEnrolled ? "text-[#31c7c8]" : "text-[#1c81ff]"}`}>
