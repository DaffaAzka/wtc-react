...
export default function ModalManageTracks({ data, isOpen, onOpenChange }: { data: StudyClass | null; isOpen: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  const handleRemove = (trackId: number) => {
    ...
    removeTrack.mutate({ studyClassId: data.id, trackId }, { onSettled: () => setRemovingId(null) });
  };

  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="w-full sm:max-w-lg overflow-x-hidden" style={{ maxWidth: '32rem', overflowX: 'hidden' }}>
        <DialogHeader>
        ...
        <div className="space-y-5 pt-2 overflow-hidden">
            ...
            {tracksLoading ? (
              ...
              <div className="space-y-2">
                {(data.tracks ?? []).map((track) => (
                  <div
                    key={track.id}
                    className="flex items-center gap-2 px-3 py-2.5 rounded-xl border border-gray-100 dark:border-white/10 bg-gray-50 dark:bg-white/[0.02]"
                  >
                    <Layers className="h-3.5 w-3.5 shrink-0 text-[#1c81ff]" />
                    <span className="text-[14px] font-medium text-gray-900 dark:text-white truncate flex-1 min-w-0">
                      {track.title}
                    </span>
                    {!track.is_active && (
          ...
          {availableTracks.length > 0 && (
            <div className="space-y-2">
              <p className="text-[12px] font-bold uppercase tracking-[0.12em] text-gray-400 dark:text-gray-600">Add Track</p>
              <Select onValueChange={handleAssign} disabled={assignTrack.isPending} value="">
