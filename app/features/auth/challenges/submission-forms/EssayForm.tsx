...
export function EssayForm({ challenge, canSubmit, isSubmitting, onSubmit }: EssayFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!content.trim()) { setError("Silakan isi jawaban Anda"); return; }
    const minWords = 10;
    if (wordCount < minWords) { setError(`Jawaban harus minimal ${minWords} kata (saat ini: ${wordCount} kata)`); return; }
  ...
  return (
    <div className="rounded-2xl bg-white border border-gray-200 dark:bg-[#0b1215] dark:border-white/10 shadow-sm overflow-hidden">
      <div className="flex items-center gap-3 px-6 py-4 border-b border-gray-100 dark:border-white/5">
        ...
        <span className="font-bold text-gray-900 dark:text-white">Tulis Jawaban Essay</span>
      </div>
        ...
        {!canSubmit ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="space-y-2">
              <label htmlFor="essay-content" className="text-[13px] font-bold text-gray-700 dark:text-gray-300 block">
                Jawaban Anda
              </label>
              <textarea
                id="essay-content"
              ...
              {challenge.settings?.explanation && (
                <div className="rounded-xl bg-[#1c81ff]/5 border border-[#1c81ff]/15 p-4 mt-2">
                  <p className="text-[12px] font-bold uppercase tracking-[0.12em] text-gray-400 dark:text-gray-500 mb-1">Kriteria Penilaian</p>
                  <p className="text-[13px] text-gray-600 dark:text-gray-300">{challenge.settings.explanation}</p>
                </div>
              )}
            </div>
