...
export function QuizGroupForm({
  ...
  const handleMCQAnswer = (questionIndex: number, answer: string) =>
    setAnswers((prev) => [
      ...
      { questionIndex, type: "multiple_choice", answer },
    ]);

  const handleEssayAnswer = (questionIndex: number, text: string) =>
    setAnswers((prev) => {
      const f = prev.filter((a) => a.questionIndex !== questionIndex);
      return text.trim()
        ? [...f, { questionIndex, type: "essay", answer: text }]
        : f;
    });

  const getAnswer = (questionIndex: number) =>
    answers.find((a) => a.questionIndex === questionIndex)?.answer || "";

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (answers.length < questions.length) {
      setError(
        `Silakan jawab semua soal (${answers.length}/${questions.length} terjawab)`,
      );
      return;
    }
    ...
    for (const answer of essayAnswers) {
      ...
      if (wordCount < 10) {
        setError(
          `Jawaban essay harus minimal 10 kata (soal #${answer.questionIndex + 1})`,
        );
        return;
      }
    }
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between text-[12px] text-gray-400 dark:text-gray-600 tabular-nums">
                <span>Progress</span>
                <span>
                  {answers.length}/{questions.length} soal terjawab
                </span>
              </div>
                ...
                <div className="p-5 space-y-4">
                  ...
                  {question.type === "multiple_choice" && (
                    <RadioGroup
                      ...
                      onValueChange={(v) => handleMCQAnswer(index, v)}
                      className="space-y-2">
                        ...
                        (option: string, optIndex: number) => {
                          ...
                          return (
                            <div
                              ...
                              <Label
                                htmlFor={`q${index}-${key}`}
                                className="flex-1 cursor-pointer text-[14px] text-gray-700 dark:text-gray-300">
                                <span
                                  className={`font-extrabold mr-2 ${isSelected ? "text-[#1c81ff]" : "text-gray-400 dark:text-gray-600"}`}>
                                  {key}.
                                </span>
                                {option}
                              </Label>
                  ...
                  {question.type === "essay" && (
                    <div className="space-y-2">
                      ...
                      <textarea
                        value={getAnswer(index)}
                        onChange={(e) =>
                          handleEssayAnswer(index, e.target.value)
                        }
