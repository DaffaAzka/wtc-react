...
export function actionBadgeConfig(action: string): ActionBadgeConfig {
  ...
  if (a === "created")
    return { label: "created", bg: "bg-[#00E676]/10", text: "text-[#00E676]" };
  if (a === "updated")
    return { label: "updated", bg: "bg-[#1c81ff]/10", text: "text-[#1c81ff]" };
  if (a === "deleted")
    return { label: "deleted", bg: "bg-[#ff007b]/10", text: "text-[#ff007b]" };
  if (a === "restored")
    return { label: "restored", bg: "bg-[#31c7c8]/10", text: "text-[#31c7c8]" };
  return {
    label: action,
...
function ActionBadge({ action }: { action: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-[11px] font-bold uppercase tracking-[0.08em] capitalize ${cfg.bg} ${cfg.text}`}>
      {cfg.label}
    </span>
  );
}
...
function ActorCell({ actor }: { actor: AuditLog["actor"] }) {
  if (!actor) {
    return (
      <span className="text-[12px] text-gray-400 dark:text-gray-600 italic">
        System
      </span>
    );
  }
            ...
            {isLoading ? (
              ...
              logs.map((log) => (
                <tr
                  ...
                  <td className="hidden px-5 py-3.5 lg:table-cell">
                    <span
                      className="text-[13px] text-gray-500 dark:text-gray-400 font-mono"
                      title={
                        log.changed_fields
