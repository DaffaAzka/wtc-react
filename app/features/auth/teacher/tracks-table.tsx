...
import ModalEdit from "@/features/auth/tracks/modal-edit";
import {
  EllipsisIcon,
  Inbox,
  RefreshCw,
...
type TrackWithCreator = Track & { creator?: Creator | null };

interface TeacherTracksTableProps {
  data: TrackWithCreator[];
  loading?: boolean;
...
export default function TeacherTracksTable({
  data,
  loading = false,
  error = null,
  onRetry,
  total,
}: TeacherTracksTableProps) {
  const [search, setSearch] = useState("");
  const [editModal, setEditModal] = useState<{ data: Track | null; isOpen: boolean }>({ data: null, isOpen: false });
  const [deleteAlert, setDeleteAlert] = useState<{ data: TrackWithCreator | null; isOpen: boolean }>({ data: null, isOpen: false });
  ...
  const handleConfirmDelete = () => {
    ...
    deleteTrack.mutate(deleteAlert.data.slug, {
                ...
                <tr key={track.id} className="group hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-2">
                      ...
                      <code className="hidden sm:inline rounded-lg bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 px-1.5 py-0.5 font-mono text-[11px] text-gray-400 dark:text-gray-600">/{track.slug}</code>
                    </div>
                    ...
                    <div className="flex items-center justify-end opacity-0 group-hover:opacity-100 transition-opacity">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="rounded-xl">
                          ...
                          <DropdownMenuItem variant="destructive" className="rounded-lg" onClick={() => setDeleteAlert({ data: track, isOpen: true })}>
                            Delete
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
