...
import ModalEdit from "@/features/auth/modules/modal-edit";
import {
  EllipsisIcon,
  Inbox,
  RefreshCw,
...
type ModuleWithCreator = Module & { creator?: Creator | null };

interface TeacherModulesTableProps {
  data: ModuleWithCreator[];
  loading?: boolean;
...
export default function TeacherModulesTable({
  data,
  loading = false,
  error = null,
  onRetry,
  total,
}: TeacherModulesTableProps) {
  const [search, setSearch] = useState("");
  const [editModal, setEditModal] = useState<{ data: Module | null; isOpen: boolean }>({ data: null, isOpen: false });
  const [deleteAlert, setDeleteAlert] = useState<{ data: ModuleWithCreator | null; isOpen: boolean }>({ data: null, isOpen: false });
  ...
  const handleConfirmDelete = () => {
    ...
    deleteModule.mutate(deleteAlert.data.slug, {
              ...
              {filtered.map((module) => (
                <tr key={module.id} className="group hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                  ...
                  <td className="hidden px-5 py-3.5 lg:table-cell">
                    <CreatorBadge creator={module.creator} />
                  </td>
                    ...
                    <div className="flex items-center justify-end opacity-0 group-hover:opacity-100 transition-opacity">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="rounded-xl">
                          ...
                          <DropdownMenuItem variant="destructive" className="rounded-lg" onClick={() => setDeleteAlert({ data: module, isOpen: true })}>Delete</DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </div>
                  </td>
