    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center gap-3 mb-5">
          ...
          <span className="font-bold text-gray-900 dark:text-white">Pencapaian</span>
        </div>
  ...
  if (error) {
    return (
      <div className="flex items-start gap-3 rounded-2xl bg-red-50 dark:bg-red-500/5 border border-red-200 dark:border-red-500/20 p-4">
        ...
        <p className="text-[14px] text-red-600 dark:text-red-400">Gagal memuat pencapaian</p>
      </div>
    );
  }
    ...
    <div className="space-y-5">
      ...
      {!achievements || achievements.length === 0 ? (
        <div className="flex flex-col items-center gap-4 py-14 text-center rounded-2xl border border-dashed border-gray-200 dark:border-white/10">
          ...
          <div>
            <p className="text-[15px] font-bold text-gray-900 dark:text-white">Belum Ada Pencapaian</p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {achievements.map((achievement) => (
            ...
            >
              ...
              {/* Info */}
              <div>
                <h4 className="font-extrabold text-[13px] text-gray-900 dark:text-white leading-tight mb-1" style={{ letterSpacing: "-0.01em" }}>
                  {achievement.title}
                </h4>
              ...
              <span className="inline-flex items-center gap-1 rounded-full bg-[#f6b60b]/10 px-2.5 py-0.5 text-[11px] font-bold text-[#f6b60b]">
                ...
                {format(new Date(achievement.earned_at), "dd MMM yyyy", { locale: localeId })}
              </span>
            </div>
          ))}
        </div>
