...
export default function AllChallengesPage() {
  const { challenges, loading, error, refresh } = useGetChallenges();
    ...
    return (
      <>
        ...
        <ErrorState
          ...
          message={error.message || "An error occurred while loading challenges."}
          onRetry={refresh}
        />
      </>
    );
    ...
    <>
      <div className="mb-6">
        ...
        <p className="mt-0.5 text-sm text-muted-foreground">
          Browse all challenges across lessons and modules.
        </p>
      </div>
              ...
              {challenges.map((challenge) => (
                <tr key={challenge.id} className="border-b hover:bg-muted/50 group">
                  ...
                  <td className="hidden px-4 py-3 text-sm text-muted-foreground md:table-cell">
                    {challenge.points ?? challenge.max_score ?? "—"}
                  </td>
                  <td className="px-4 py-3 text-sm text-muted-foreground">
                    {challenge.lesson_id ? (
                      <span>Lesson #{challenge.lesson_id}</span>
                    ) : challenge.module_id ? (
                      <span>Module #{challenge.module_id}</span>
                    ) : (
                      "—"
                    )}
                  </td>
